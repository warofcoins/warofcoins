...
unsigned int nTargetSpacing = 90; // 90 seconds block spacing
unsigned int nStakeMinAge = 60 * 60 * 8; //8 hours minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 14;
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed

const int64_t nChainStartTime = 1426852800;
const int64_t nTestNetStartTime = nChainStartTime;
...
const string strMessageMagic = "WarOfCoins Signed Message:\n";
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        static const CWarOfCoinsAddress lostWallet ("Sg2NdZywasiNUADzm5dxt6G1WWKyKkj9Mc");
        uint256 hashBlock;
        CTransaction txPrev;
        ...
        if(GetTransaction(txin.prevout.hash, txPrev, hashBlock)){ // get the vin's previous transaction
            ...
            if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source)){ // extract the destination of the previous transaction's vout[n]
                CWarOfCoinsAddress addressSource(source);
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    // fMiner is true when called from the internal WarOfCoins miner
...
bool CBlock::AcceptBlock()
{
    ...
    if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()) && nHeight > LAST_POW_BLOCK)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WarOfCoins", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "WOC is the winning coin";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:WOC genesis block";
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x72787e68be70217a5126871f13d500f8fb067ed46bfc94d18b7aeabd4cda8927"));
